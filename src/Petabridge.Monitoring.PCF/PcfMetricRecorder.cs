...
namespace Petabridge.Monitoring.PCF
{
    /// <inheritdoc />
    /// <summary>
    ///     A <see cref="T:Petabridge.Monitoring.PCF.IPcfMetricRecorder" /> implementation that uses Akka.NET actors to aggregate
    ...
    public sealed class PcfMetricRecorder : IPcfMetricRecorder, IDisposable
    {
        ...
        public static PcfMetricRecorder Create(PcfMetricForwarderSettings settings, ActorSystem system = null)
        {
            ...
            var counterActor = system.AsInstanceOf<ExtendedActorSystem>().SystemActorOf(
                Props.Create(() => new CounterAggregator(reporterActor, settings.TimeProvider ?? new DateTimeOffsetTimeProvider())), $"pcf-reporter-{NameCounter.GetAndIncrement()}");
        ...
        /// </summary>
        internal PcfMetricRecorder(IActorRef reporterActorRef, IActorRef counterAggregatorRef, ActorSystem ownedActorSystem) : this(PcfMetricForwarderSettings.FromEnvironment(), reporterActorRef, counterAggregatorRef, ownedActorSystem)
        {
            
        }

        internal PcfMetricRecorder(PcfMetricForwarderSettings settings, IActorRef reporterActorRef, IActorRef counterAggregatorRef, ActorSystem ownedActorSystem = null)
        {
            Settings = settings;
        ...
        {
            _counterAggregatorRef.Tell(new CounterAggregator.CounterIncrement(ApplyPostfix(name, CounterPostfix), value));
        }
        ...
        {
            _reporterActorRef.Tell(new PcfMetricRecording(ApplyPostfix(name, TimingPostfix), value, "milliseconds", TimeProvider.NowUnixEpoch, null));
        }
